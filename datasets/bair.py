...
# https://github.com/voletiv/mcvd-pytorch/blob/master/datasets/bair.py
...
class BAIRDataset(Dataset):
    ...
    def __init__(self, data_path, frames_per_sample=5, random_time=True, random_horizontal_flip=True, color_jitter=0,
                 total_videos=-1, with_target=False):
...
if __name__=='__main__':
    ...
    path = 'datasets/BAIR_h5/train'
    
    # Read from dir_path which contains the HDF5_file
    h5_ds = BAIRDataset(path, frames_per_sample=5, random_time=True, )
    data = h5_ds[0]
    print(data[0].shape)  
    ...
    for i, data in enumerate(h5_ds):
#        fig = plt.figure()
        if i==2:
            break
        for frame in data[0]:
            frames.append([plt.imshow(torch.permute(frame, (1,2,0)), animated=True)])

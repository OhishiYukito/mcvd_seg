# https://github.com/voletiv/mcvd-pytorch/blob/master/models/unet.py
...
def get_timestep_embedding(timesteps, embedding_dim: int = 128):
    ...
    emb = math.log(10000) / (half_dim - 1)    # or 'emb = math.log(10000) / half_dim'
    emb = torch.exp(torch.arange(half_dim, dtype=torch.float, device=timesteps.device) * -emb)  # or 'torch.arrange(half_dim, ...' -> 'torch.arrange(1, half_dim+1, ...'
    emb = timesteps.float().unsqueeze(1) * emb.unsqueeze(0)
    emb = torch.cat([torch.sin(emb), torch.cos(emb)], dim=1)    # [sin(i), sin(i+1), ..., cos(i), cos(i+1), ...]
...
class UNet(nn.Module):
    ...
    def forward(self, x, t=None, cond=None):
        ...
        if t is not None and self.time_conditional:
